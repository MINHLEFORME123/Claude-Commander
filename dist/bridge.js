...
export class GeminiBridge {
    ...
    async sendToSession(sessionId, userPrompt) {
    ...
    async compactSession(sessionId) {
        ...
        try {
            ...
            return this.store.updateSession(sessionId, (currentSession) => {
                ...
                currentSession.messages = currentSession.messages.filter((message) => message.role === "system").concat(currentSession.messages.filter((message) => message.role !== "system").slice(-this.config.recentMessageLimit));
    ...
    buildConversationPrompt(session, userPrompt) {
        const sections = [this.config.systemPrompt.trim()];
