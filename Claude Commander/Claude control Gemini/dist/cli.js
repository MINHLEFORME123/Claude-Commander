...
function renderSessionOverviewList(sessions) {
    if (sessions.length === 0) {
        return "No sessions found.";
    }
    const lines = [];
    for (const session of sessions) {
        lines.push(`- ${session.id} | ${session.status} | ${session.updatedAt} | ${session.messageCount} messages | ${session.name}`);
...
function renderSessionRecord(session, recentLimit) {
...
function parseSessionStartArgs(argv) {
    const nameParts = [];
    let explicitName;
...
function parseSessionIdAndMessage(argv) {
...
async function handleConfigCommand(config, argv, options) {
...
async function handleSessionCommand(bridge, recentLimit, argv, options) {
    const [subcommand, ...rest] = argv;
    switch (subcommand) {
        case "start": {
...
async function main() {
    ...
    const configLoadOptions = { baseDir: options.cwd ?? process.cwd() };
